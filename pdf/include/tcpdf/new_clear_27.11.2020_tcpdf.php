...
class MATH_TCPDF {
	...
	public function Footer() {
        ...
        $inch_poster_left_margin = 0.25;
        $inch_poster_right_margin = 0.25;
        $inch_poster_top_margin = 0.5;
        
        $x_left_margin_add = 0.5; 
        $width_copyright = 4.9;//3.28; // 4.9  
        
        $color_cmyk = array(2,2,0,50);   
        $this->SetTextColorArray($color_cmyk);
        
        
        
         // Position at 15 mm from bottom
         // 1. ====================== 1 Footer PAGES NUMBER ================ 
        $this->SetY(-0.5);
        // Set font
        $this->SetFont('helvetica', '', 10);
        // Page number Page 1 of 12
        $this->Cell(0, 0.5, 'Page '.$this->getAliasNumPage().' of '.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
        // 1. ====================== 1 Footer PAGES NUMBER ================
        
       
        
        
       // 2 ------------------------------- Footer ADD Tst StickTogether  ----------------------------- 
        $footer_text_year_now = date('Y',time() ); //  2018            
        $footer_text_url_link = 'https://www.letsticktogether.com/';
        $footer_text_title_url_stick_website = 'www.letsticktogether.com'; 
        $html_footer_text_one_left = '© '.$footer_text_year_now.' StickTogether Products, LLC';
   
        
        
        
         // 1. Footer TeXT _ @ 2020 StickTogether Products, LLC - In LEFT
         $width_copyright_footer = 3;
         $x_footer_text_one = $inch_poster_left_margin  ;// $inch_poster_left_margin + $x_left_margin_add; //+ $inch_matrix_grid_width - $width_copyright;
         $y_footer_text_one = $inch_poster_height - $inch_poster_top_margin ;
         //$this->SetFont('helvetica', '', 12); 
         
        $this->SetXY($x_footer_text_one, $y_footer_text_one); 
        $this->Cell(  $w=$width_copyright_footer, $h=0.5, $txt=$html_footer_text_one_left, $border=0, $ln=0, $align='L', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='R', $valign='M');
         // 1 ---------------------------------------------------------
         ...
         // 2. Footer TeXT _ www.letcticktogether.com - In RIGHT
         $width_links_footer = 2;
         $x_footer_text_one = $inch_poster_width - $width_links_footer - $inch_poster_right_margin  ;// $inch_poster_left_margin + $x_left_margin_add; //+ $inch_matrix_grid_width - $width_copyright;
         $y_footer_text_one = $inch_poster_height - $inch_poster_top_margin ;
         //$this->SetFont('helvetica', '', 12); 
         
        $this->SetXY($x_footer_text_one, $y_footer_text_one); 
        $this->Cell(  $w=$width_links_footer, $h=0.5, $txt=$footer_text_title_url_stick_website, $border=0, $ln=0, $align='R', $fill=false, $link=$footer_text_title_url_stick_website, $stretch=0, $ignore_min_height=false, $calign='R', $valign='M');
         // 2 -----------------------------------------------------
      
      
        
         // 3. Footer TeXT _ Collection name - In RIGHT ST-MATH-123456
         // ВАЖНО НАЗВАНИЕ БРАТЬ ИЗ ГЛОБАЛЬНОЙ ПЕРЕМЕННОЙ
         $footer_text_collection_name = 'ST MATH-123456';    
         $width_links_footer = 1.8;
         $add_left_padding = 0.2;
         $x_footer_text_one = ($inch_poster_width / 2) + $add_left_padding  ;// $inch_poster_left_margin + $x_left_margin_add; //+ $inch_matrix_grid_width - $width_copyright;
         $y_footer_text_one = $inch_poster_height - $inch_poster_top_margin ;
         //$this->SetFont('helvetica', '', 12); 
         
        $this->SetXY($x_footer_text_one, $y_footer_text_one); 
        $this->Cell(  $w=$width_links_footer, $h=0.5, $txt=$footer_text_collection_name, $border=0, $ln=0, $align='C', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='R', $valign='M');
         // 3 -----------------------------------------------------
	 ...
	 * @param $txt (string) String to print
	 * @param $border (mixed) Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(222, 22, 222)))
