...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  if (isset($_REQUEST['data'])) {
    if ($_REQUEST['data']) {
      $filename = ($_REQUEST['data']); // clear_string
    }
  }

  // if (!is_readable("./data/" . $filename . ".json")) {
  //   die('Can`t read');
  // }
  ...
  if ($dirs and $tiles) {
    ...
    $pdf->Text($x_init * 3 + 0.84, 0.1, $txt = 'Assemble the 300 pieces using this Image Key as a guide.', false, false, true, 0, 0, '', false, '', 0, false, 'T', 'M', false);
    $pdf->Text($x_init * 3 + 0.02, 0.3, $txt = 'Make sure to attach each piece in the correct location and orientation.', false, false, true, 0, 0, '', false, '', 0, false, 'T', 'M', false);
    $pdf->Text($x_init * 3 + 0.74, 0.5, $txt = 'When all 300 pieces are connected, flip to reveal the image.', false, false, true, 0, 0, '', false, '', 0, false, 'T', 'M', false);
    $pdf->Text($x_init * 3 + 1.68, 0.7, $txt = 'Puzzle Face’s are best viewed from a distance.', false, false, true, 0, 0, '', false, '', 0, false, 'T', 'M', false);
